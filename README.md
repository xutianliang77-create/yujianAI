...
- P2-01/02/03 已在 Beelink 以独立 `yujian-p2` Compose project 部署 PostgreSQL 16.4、Redis
  7.2.7 和 OpenBao 2.4.1；数据持久化到 `/home/beelink/yujianAI/data/p2`，8 条 migration、
  runtime adapter、原子 Redis 限流、KMS secret round-trip 和重启恢复 smoke 通过。完整 P2
  仍未关闭：注册/邀请/SSO/RBAC、Webhook 真实投递、多副本竞争、备份恢复和数据权利执行尚待验收。
