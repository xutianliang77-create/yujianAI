...
用户确认：Beelink 是服务器端并配置 1 块 RTX 5090；所有构建、测试、Docker、浏览器、
Flutter、LiveKit/RTC、GPU 和集成验收等运行操作继续等待 Beelink 开机，本轮只做源码、合同、
部署边界和静态完整性检查。
...
1. Beelink 开机后运行 `npm run beelink:preflight`，确认唯一 RTX 5090、Docker、Node、Flutter、
   Chrome、双 RTC 节点和网络条件。
2. 运行 `npm run beelink:acceptance`，先修复合同/构建/服务启动失败，再做 Web/Flutter 音频
   Track 发布订阅、双 LiveKit 节点、Agent GPU 和媒体链路验收。
3. 部署侧提供 `resourceUsage`、PostgreSQL/KMS/Redis runtime module；随后再做持久化、分布式
...
2. 执行 `npm run beelink:acceptance`，记录双 RTC、Web/Flutter 音频 Track、Agent 5090 和媒体链路证据。
