...
if (Number(migrations.rows[0]?.count) !== 8) throw new Error("PostgreSQL migration state did not survive restart");
if (cleanup) {
...
await pool.end();
console.log(JSON.stringify({ platformApi: "snapshot-restored", redis: "sentinel-restored-after-rebuild", postgres: "8-migrations-restored", cleanup: cleanup ? "complete" : "deferred" }));
