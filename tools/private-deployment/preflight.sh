...
for migration in 001_platform.sql 002_domain_expansion.sql 003_agent_control.sql 004_media_ops.sql 005_outbox_delivery.sql 006_platform_store.sql 007_webhook_destinations.sql 008_rtc_telemetry.sql 009_p2_closure.sql 010_data_rights_recovery.sql 011_outbox_claim_ownership.sql 012_preview_operations.sql 013_agent_runtime.sql 014_media_accounting.sql 015_private_deployment.sql 016_ga_commerce.sql; do
...
helm lint "$chart_dir" \
  --set rtc.primaryWsUrl=ws://placeholder.invalid \
  --set platformRuntime.modulePath=/etc/yujian/platform-runtime.mjs \
  --set platformRuntime.postgresSecretName=yujian-platform-data \
  --set platformRuntime.postgresSecretKey=postgres-url \
  --set platformRuntime.redisSecretName=yujian-platform-data \
  --set platformRuntime.redisSecretKey=redis-url \
  --set 'platformRuntime.externalDataEgressCidrs[0]=127.0.0.1/32' \
  --set 'platformRuntime.kmsEgressCidrs[0]=127.0.0.1/32' >/dev/null
