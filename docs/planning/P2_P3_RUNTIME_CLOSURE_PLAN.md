...
三节点 HTTPS/Raft 和 API key 生命周期验收。P2-04/05/06 实现及第 9 条 migration
...
| P2-06 | 备份恢复和数据权利执行器 | **implementation-ready / not production-accepted**：PostgreSQL executor/worker、0600 prepared→committed evidence、isolated `pg_dump` restore、Redis 从 PG 真值重建脚本已完成；本地 2/2 data-rights 测试通过；死机前未执行到真实备份恢复 | PostgreSQL restore、Redis 重建、删除/导出 evidence 和 RPO/RTO |
