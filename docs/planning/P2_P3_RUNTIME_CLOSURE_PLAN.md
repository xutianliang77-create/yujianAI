...
| P2-04 | 注册、邀请、SSO/OIDC、onboarding、持久化 RBAC | **implementation-ready / acceptance-interrupted**：新增 OIDC 自助 onboarding、邀请接受、PostgreSQL member scope resolver、持久角色优先于 token role claim、跨 tenant 拒绝与双机 RTC probe；本地 20/20 platform-api 测试通过；最终本机客户端 join 被 Beelink 死机中断 | 新用户到第一条 Room、跨 tenant IDOR、审计报告的单次完整脱敏报告 |
