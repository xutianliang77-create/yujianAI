...
版本：v1.0  
日期：2026-07-17  
状态：P2-01/02/03 Beelink 生产验收通过；P2 完整 Gate 未关闭；P3 gate-locked
...
并完成 8 条 migration、runtime adapter 构建、事务 outbox/CAS、Redis 多客户端竞争、OpenBao
三节点 HTTPS/Raft、API key 生命周期和重启/重建恢复验收。身份 provider、注册邀请、持久化
RBAC、Webhook 真实投递、备份恢复和数据权利执行器仍未完成，不能把本次部署写成 P2 Gate 已关闭。
