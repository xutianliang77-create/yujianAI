...
版本：v1.0  
日期：2026-07-17  
状态：P1-M0-03 运行证据已补；P1-M0-04 当前/候选真实证据已生成，仅 Redis 候选可进回归，漏洞/许可证/Owner 仍阻断；完整 Gate 0/1 未通过
...
| P1-M0-04 | LICENSE/NOTICE、SBOM、漏洞与签名策略 | 当前 4 镜像证据为 76 Critical/465 `NOASSERTION`；候选只拉取扫描、未部署，Redis 7.2.14 为零 Critical 可进回归，PostgreSQL 16.14 两选项为 27/1、OpenBao 2.5.4 为 13，仍阻断；四类 Owner 已指定为 aaa/bbb/ccc/ddd，eee 批准，联系/备份/本人签字待补 | `legal-owner` + `release-owner` + `security-owner` + `compliance-owner` | 当前/候选 evidence JSON、mode 0600 原始报告、修复后零 Critical、完整许可证和个人签字 |
