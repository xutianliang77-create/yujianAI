...
| M2-03 API key/KMS/轮换 | partial | 只显示一次、hash 校验、轮换/撤销、API key metadata/list GET、可配置双 key grace period、Local envelope KMS adapter、SQL schema、snapshot 只保存 hash | 外部 KMS、secret registry、真实恢复演练未完成 |
...
| M2-05 Region/Quota | partial | region/residency/capacity router、token quota snapshot、可注入 `PlatformResourceUsageProvider`/`PlatformTokenQuotaProvider`、`CompositePlatformResourceUsageProvider` 字段 owner 合并校验、Redis token reservation/lease adapter | participant/Room/data/媒体实时 provider wiring、生产 Redis runtime 接线未完成 |
...
**M2 结论：partial；Gate 2 未通过。** 控制面最小 API、迁移执行器、runtime module 和
adapter 骨架已实现，但仍缺外部 PostgreSQL/KMS/Redis wiring、分布式 quota、webhook、SSO
和 Beelink 运行证据。
